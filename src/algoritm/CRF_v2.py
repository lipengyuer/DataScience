        ...
        for i in range(1, t):
            ...
            for n in range(len(self.hiddenStatList)):#遍历本步的所有隐藏状态
                ...
                for j in range(stateNumOfFormerStep):
                    ...
                    transProbMatrix[j, n] = self.getSumOfFeatureFuctions(thisState, formerState, thisObservation)
...
#         print("计算状态转移特征边缘概率的耗时是", tt3)
    ...
    #                 print(gradMap.get(featureName, 0)) 
                    grad = grad - 1*self.calFeatureFunctionValueAndMargProb(featureName, charList, t)/z_x - \
                                 self.featureWeightMap.get(featureName, 0)/(corpusSize * 1)
                    t2 = time.time()
                    tt2 += t2-t1
                possibleFeatureNames = self.generatePossibleStateTrans(t)               
                if featureName in possibleFeatureNames:
    ...
    def fit(self, sentenceList):
        ...
        for epoch in range(self.epoch):
            ...
            for n in range(batchSize):
                ...
                result = pool.apply_async(calGrad4WeightSlow4Multi, args=(self, sentence, batchSize, n))
...
#计算模板函数权重对应的梯度
def calGrad4WeightSlow4Multi(self, sentence, corpusSize, n):
#     print("正在计算第", n, '句的梯度')
    charList = '@' + sentence[0] + '@'
    tagList = '*' + sentence[1] + '#'
    sentenceLength = len(charList)
...
#     print(sentenceList)
    ...
    if preTrain:
        ...
        model = LinearChainCRF(epoch=100, learningRate=0.00001, workerNum = 20)
