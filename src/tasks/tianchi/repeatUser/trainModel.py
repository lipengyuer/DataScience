...
def featureEngineering(df, fileName):
    ...
    #读取用户画像
    personas_df = personas.personas4User(fileName)
    df = df.merge(personas_df, on=['user_id'], how='left')
...
def  dataProcess():
#     fileName = r"data_format2\\train_format2_sub.csv"
    fileName = "data_format2/train_format2_sub.csv"
    df = pd.read_csv(fileName)
    X,Y = featureEngineering(df, fileName)
    return X, Y
...
def KFoldValidation(X, Y, K=10):
    kfolder = KFold(n_splits=K, random_state=int(time.time()))
    totalAUC, totaltrain_auc = 0, 0
    count = 0
...
#         clf = LogisticRegression(penalty='l1', C=0.9, class_weight='balanced')
#         clf = tree.DecisionTreeClassifier()
        #clf = ensemble.RandomForestRegressor(n_estimators=50, max_depth=8, max_features=0.6)
        clf = ensemble.GradientBoostingRegressor(learning_rate=0.01, n_estimators=50,subsample =0.8,
                                                 max_depth=9, max_features=0.6)
...
#         clf = SVC()
        ...
        totalAUC += test_auc
        totaltrain_auc += train_auc

    print('avg AUC is:', totalAUC/K, '训练集中的为', totaltrain_auc/K)

def  getIDs():
    print("读取id数据。")
    id_set = set({})
    fileName = r"sample_submission.csv"
...
def getFinalResult(clf):
    id_set = getIDs()
    fileName = r"data_format2/test_format2.csv"
    data = []
    count = 0
    with open(fileName, 'r') as f:
        line = f.readline()
    ...
    X, _ = featureEngineering(df, fileName)
    #print(X)
    Y = clf.predict(X)
