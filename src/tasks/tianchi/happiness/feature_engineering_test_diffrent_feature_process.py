...
def featureEngineering(data):
    global features
    data['null_col_num'] = data.apply(lambda x: len(list(filter(lambda y: y!=None, x))))
    ...
    data['edu_yr'] = data['edu_yr'].apply(lambda x: 2014 if x < 0 else x)
    data['edu_yr'] = data['edu_yr'].apply(lambda x: 2015-x if x > 100 else min(x, 10))
    data['high_edu_1'] = data['edu_yr'].apply(lambda x: 1 if x>13 else 0)
    data['high_edu_2'] = data['edu_yr'].apply(lambda x: 1 if x>15 else 0)
    data['age_highest_edu'] = data['edu_yr'] - data['realBirth']
    ...
    data['edu'] = data['edu'].apply(lambda x: 4 if x<0 else x)
...
#     data['car_have']  = data['car'].apply(lambda x: 1 if x==1  else 0) 
    ...
    stopFeatures = ['invest_other', 'property_other', 'edu_other', 'survey_time', 'realBirth'\
                    ...
                    ,'m_work_14', 'nationality', 'car']
    data = data.drop(stopFeatures, axis=1)

    data = aug_data_age_group(data)
    if features==None:
        features = []
