...
userStasticsCollection = "completeUserFeatureSample_adv"
...
def ngramFreatures(sampleSize, one_hot=True):
    featureName = ["wordFreq"]#, 'postagBigramFreq']
    data = collection.find({}, {'_id':1, "gender": 1, "wordFreq":1})#, 'postagBigramFreq':0})#从mongo中查询这个特征以及对应的性别标签
    dataList = []
    print("正在读取数据")
    count = 0
    vocab = {}
    ...
    for line in data:
        ...
        for fieldName in featureName:
            # print(fieldName, line[fieldName])
            for word in line[fieldName]:
                tempData[word[0]] = word[1]
            # line[featureName] = tempData
        tempData['gender'] = line['gender']
...
if __name__ == '__main__':
    ...
    for one_hot_stat in one_hot_list:
        ...
        #获取训练集和测试集
        trainData, testData, vocabSize = ngramFreatures(-1,
                                                        one_hot = False)
        # print(trainData[:2])
        time.sleep(20)
        #将本地数据放到集群中
