...
#         if self.mode=='train':
        ...
        self.filter_1_w = tf.Variable(tf.random_normal([\
                            2, self.parameters['embedding_size'], 1, node_num], mean=0, stddev=0.01),name='filter_1_w')
        self.filter_1_b = tf.Variable(tf.random_normal([node_num], mean=0, stddev=0.1), name='filter_1_b')
        conv_1 = tf.nn.conv2d(X_char_vec4cnn, self.filter_1_w, strides=[1, 1, 1, 1], padding='VALID')
        hidden_out_1 = tf.nn.tanh(tf.nn.bias_add(conv_1, self.filter_1_b))
        pooled_out_1 = tf.nn.max_pool(hidden_out_1, \
                                      ksize=[1, self.parameters['max_text_length'] - 2 + 1 - k, 1, 1],\
                                      strides = [1, 1, 1, 1], padding='VALID')
        print("pooled_out_1 size is ", pooled_out_1)
        flatten_out_1 = tf.reshape(pooled_out_1, [-1, node_num*(k+1)])

        #2号卷积核
        self.filter_2_w = tf.Variable(tf.random_normal(\
                                [3, self.parameters['embedding_size'], 1, node_num], mean=0, stddev=0.01), name='filter_2_w')
        self.filter_2_b = tf.Variable(tf.random_normal([node_num], mean=0, stddev=0.1), name='filter_2_b')
        conv_2 = tf.nn.conv2d(X_char_vec4cnn, self.filter_2_w, strides=[1, 1, 1, 1], padding='VALID')
        hidden_out_2 = tf.nn.tanh(tf.nn.bias_add(conv_2, self.filter_2_b))
        pooled_out_2 = tf.nn.max_pool(hidden_out_2, \
                                      ksize=[1, self.parameters['max_text_length'] - 3 + 1 - k, 1, 1],\
                                      strides = [1, 1, 1, 1], padding='VALID')
        print("pooled_out_2 size is ", pooled_out_2)
        flatten_out_2 = tf.reshape(pooled_out_2, [-1, node_num*(k+1)])
        
        #3号卷积核
        self.filter_3_w = tf.Variable(tf.random_normal(\
                        [4, self.parameters['embedding_size'], 1, node_num], mean=0, stddev=0.01), name='filter_3_w')
        self.filter_3_b = tf.Variable(tf.random_normal([node_num], mean=0, stddev=0.1), name='filter_3_b')
        conv_3 = tf.nn.conv2d(X_char_vec4cnn, self.filter_3_w, strides=[1, 1, 1, 1], padding='VALID')
        hidden_out_3 = tf.nn.tanh(tf.nn.bias_add(conv_3, self.filter_3_b))
        pooled_out_3 = tf.nn.max_pool(hidden_out_3, \
                                      ksize=[1, self.parameters['max_text_length'] - 4 + 1 - k, 1, 1],\
                                      strides = [1, 1, 1, 1], padding='VALID')
        print("pooled_out_2 size is ", pooled_out_3)
        flatten_out_3 = tf.reshape(pooled_out_3, [-1, node_num*(k+1)])

        #4号卷积核
        self.filter_4_w = tf.Variable(tf.random_normal(
                        [5, self.parameters['embedding_size'], 1, node_num], mean=0, stddev=0.01), name='fiter_4_w')
        self.filter_4_b = tf.Variable(tf.random_normal([node_num], mean=0, stddev=0.1), name='filter_4_b')
        conv_4 = tf.nn.conv2d(X_char_vec4cnn, self.filter_4_w, strides=[1, 1, 1, 1], padding='VALID')
        hidden_out_4 = tf.nn.tanh(tf.nn.bias_add(conv_4, self.filter_4_b))
        pooled_out_4 = tf.nn.max_pool(hidden_out_4, \
                                      ksize=[1, self.parameters['max_text_length'] - 5 + 1 - k, 1, 1],\
                                      strides = [1, 1, 1, 1], padding='VALID')
        print("pooled_out_2 size is ", pooled_out_3)
        flatten_out_4 = tf.reshape(pooled_out_4, [-1, node_num*(k+1)])
        ...
        #拼接
        flatten_out = tf.concat([flatten_out_1, flatten_out_2, flatten_out_3, flatten_out_4], axis=1)
        #拉直
        #dropout
        flatten_out_droped = tf.nn.dropout(flatten_out, 0.5)
        #print("flatten_out_droped 的形状是" , flatten_out_droped)
        #softmax
        
        self.softmax_w = tf.Variable(tf.random_normal([node_num*4*(k+1), self.parameters['class_num']]), name='softmax_w')
        self.softmax_b = tf.Variable(tf.random_normal([self.parameters['class_num']]), name="softmax_bias")
        self.prob_dist = tf.nn.softmax(tf.matmul(flatten_out_droped, self.softmax_w) + self.softmax_b)
        ...
        self.train = tf.train.AdagradOptimizer(0.0005).minimize(self.losses)
#         self.train = tf.train.AdadeltaOptimizer(0.00005).minimize(self.losses)
#         self.train = tf.train.GradientDescentOptimizer(0.0001).minimize(self.losses)
