...
#         if self.mode=='train':
        ...
        for kernel_size in kernel_size_list:
            ...
            print(self.parameters['max_text_length'] - kernel_size + 1)
            pooled_out_1 = tf.nn.max_pool(hidden_out_1, \
                                          ksize=[1, self.parameters['max_text_length'] - kernel_size + 1 - time_step, 1, 1],\
                                          strides = [1, 1, 1, 1], padding='VALID')
            pooled_out_1 = pooled_out_1[:,:,0,:]
            print(count, "pooled_out_1 size is ", pooled_out_1)
            
            #用rnn处理序列
            lstm_cell_fw = tf.nn.rnn_cell.BasicLSTMCell(rnn_node, forget_bias=0.0,state_is_tuple=True, name='lstmf' + str(count))
            lstm_cell_fw=tf.nn.rnn_cell.DropoutWrapper(lstm_cell_fw,output_keep_prob=0.5)
            lstm_cell_bw = tf.nn.rnn_cell.BasicLSTMCell(rnn_node, forget_bias=0.0,state_is_tuple=True, name='lstmb' + str(count))
            ...
            print(count, output, node_num*node_num*(1 + time_step))
            output = tf.reshape(output, [-1, 2*rnn_node*(1 + time_step)])
            output = tf.nn.dropout(output, 0.6)
            
            flatten_out_list.append(output)
            
            
        
        #拼接
        flatten_out_droped = tf.concat(flatten_out_list, axis=1)
        #拉直
        #dropout
#         flatten_out_droped = tf.nn.dropout(flatten_out, 0.6)
        print("flatten_out_droped 的形状是" , flatten_out_droped, node_num*2*rnn_node*len(kernel_size_list))
        #softmax
        
        self.softmax_w = tf.Variable(tf.random_normal([2*rnn_node*(1 + time_step)*len(kernel_size_list),\
