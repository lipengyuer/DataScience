...
class AGAN():
    ...
    def gen_layer(self):
        ...
        #         gen_layer_1 = tf.layers.dense(inputs=self.fake_images, units=200, activation=tf.nn.sigmoid)
                gen_layer_2 = tf.nn.dropout(gen_layer_1, rate=self.dropout_rate)
        #         gen_layer_2 =tf.layers.batch_normalization(gen_layer_1, training=True)
                gen_layer_2 = tf.layers.dense(inputs=gen_layer_2, units=200, activation=tf.nn.sigmoid)
                gen_layer_2 = tf.nn.dropout(gen_layer_2, rate=self.dropout_rate)
        #         gen_layer_3 = tf.layers.dense(inputs=gen_layer_2, units=100, activation=tf.nn.sigmoid)
    ...
    def dis_layer(self, inputs):
        with tf.variable_scope('dis', reuse=tf.AUTO_REUSE) as scope:
            dis_layer_1 = tf.layers.dense(inputs=inputs, units=200, activation=tf.nn.leaky_relu,name='Discri1')
            dis_layer_1 = tf.nn.dropout(dis_layer_1, rate=self.dropout_rate)
#             dis_layer_1 =tf.layers.batch_normalization(dis_layer_1, training=True)
            dis_layer_2 = tf.layers.dense(inputs=dis_layer_1, units=500, activation=tf.nn.sigmoid,name='Discri2')
            dis_layer_2 = tf.nn.dropout(dis_layer_2, rate=self.dropout_rate)
#             dis_layer_3 = tf.layers.dense(inputs=dis_layer_2, units=100, activation=tf.nn.sigmoid,name='Discri3')
        ...
        for epoch in range(10000):
            ...
            for i in range(len(X_batches)):
                ...
                _, loss_d_value = self.sess.run((self.train_d, self.loss_d), \
                            feed_dict={self.X: new_data_batch, self.fake_images:random_images, self.dropout_rate: 0.5})
                _, loss_g_value = self.sess.run((self.train_g, self.loss_g), \
                                                feed_dict={self.fake_images:random_images, self.dropout_rate: 0.5})
                if (total_step + 1)%1000==0:
                    loss_g_value, loss_d_real, loss_d_fake = self.sess.run((self.loss_g, self.loss_d_real, self.loss_d_fake), \
                                                feed_dict={self.X: new_data_batch, self.fake_images:random_images, self.dropout_rate: 0})
...
#                 if (total_step + 1)%10000==0:
            ...
            gen_images = self.sess.run((self.gen_images), feed_dict={self.fake_images: test_data, self.dropout_rate: 0})
